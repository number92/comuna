...
def _comun_creation_access_state(user: User | None) -> tuple[bool, float, float]:
    ...
    can_create = bool(user and user.is_staff) or max_author_rating >= minimum_rating
...
def auth_notifications(request: HttpRequest) -> HttpResponse:
    ...
    list_qs = base_qs
    if unread_only:
        list_qs = list_qs.filter(read_at__isnull=True)

    notifications = list(list_qs.order_by("-created_at", "-id")[:limit])
...
def auth_notification_read(request: HttpRequest, notification_id: int) -> HttpResponse:
    ...
    notification = (
        SiteNotification.objects.filter(id=notification_id, user=user, is_site=True).first()
    )
    if not notification:
    ...
    unread_count = SiteNotification.objects.filter(
        user=user, is_site=True, read_at__isnull=True
    ).count()
    return JsonResponse(
        {
            "ok": True,
...
def auth_notifications_read_all(request: HttpRequest) -> HttpResponse:
    ...
    now = timezone.now()
    updated = (
        SiteNotification.objects.filter(user=user, is_site=True, read_at__isnull=True)
