...
async function userFromJwt(
  ...
  const sitePromise = client.getSite()
...
async function getNotificationCount(jwt: string, mod: boolean, admin: boolean) {
  const unreads = await getClient({ auth: jwt }).getUnreadCount()
  ...
  if (mod) {
    try {
      const reportRes = await getClient({ auth: jwt }).getReportCount({})
  ...
  if (admin) {
    try {
      const applicationRes =
        await getClient({ auth: jwt }).getUnreadRegistrationApplicationCount()
